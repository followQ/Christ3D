    ...
    public class Startup
    {
        ...
        */
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                options.AddPolicy("CanWriteStudentData", policy => policy.Requirements.Add(new ClaimRequirement("Students", "Write")));//配置策略，一个策略可以对应多个声明权限(Requirements为IList)，即一个策略是对一组权限声明的描述
                //这里的策略可以在接口Controller或接口方法上声明需要的声明权限
                //[Authorize(Policy = "ManagerOnly")] 
                options.AddPolicy("ManagerOnly", policy => policy.RequireRole("manager"));//该策略示只有manager角色才有权限
